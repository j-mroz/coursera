...
public:
    ...
    void run() {
        ...
        for (auto &entry : node->getMembersList())
            payload = appendMemberData(payload, entry);
...
class FailureDetector : public Task {
    MP1Node *node;
    static const long failTimeout = TFAIL;
    static const long removeTimeout = TREMOVE;
...
void MP1Node::handleMembersData(char *buff, uint64_t count) {
    for (auto memberIdx = 0ull; memberIdx < count; ++memberIdx) {
        ...
        updateMemberEntry({entry.id, entry.port, entry.heartbeat, getTimestamp()});
        buff += sizeof(entry);
    }
}
